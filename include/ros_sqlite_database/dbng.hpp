#ifndef _SQLITE_WAPPER_DBNG_HPP_
#define _SQLITE_WAPPER_DBNG_HPP_
...
namespace sqlite_wapper
{
    ...
    public:
        ...
        auto query(Pair pair, std::string_view oper, U &&val)
        {
            ...
            using T = typename sqlite_wapper::field_attribute<decltype(pair.second)>::type;
            return query<T>(sql);
        }
...
#define WRAPER(func)                                                                                 \
    ...
    {                                                                                                \
        ...
        for_each_l(                                                                                  \
            ...
            r = item.before(std::forward<Args>(args)...); }, std::make_index_sequence<sizeof...(AP)>{});          \
        if (!r)                                                                                      \
            return result_type{};                                                                    \
        auto lambda = [this, &args...] { return this->func(std::forward<Args>(args)...); };          \
        result_type result = std::invoke(lambda);                                                    \
        for_each_r(                                                                                  \
            tp, [&r, &result, &args...](auto &item) {\
            if(!r)\
                return;\
            if constexpr (has_after<decltype(item), result_type>::value)\
            r = item.after(result, std::forward<Args>(args)...); }, std::make_index_sequence<sizeof...(AP)>{}); \
        return result;                                                                               \
    }
...
#endif //_SQLITE_WAPPER_DBNG_HPP_

#ifndef _SQLITE_WAPPER_UTILITY_HPP_
#define _SQLITE_WAPPER_UTILITY_HPP_

#include "ros_sqlite_database/entity.hpp"
#include "ros_sqlite_database/type_mapping.hpp"
...
namespace sqlite_wapper
{
    ...
    inline auto sort_tuple(const std::tuple<Args...> &tp)
    {
        ...
        {
            auto [a, b] = tp;
            if constexpr (!std::is_same_v<decltype(a), sqlite_key_t> && !std::is_same_v<decltype(a), sqlite_auto_key_t>)
    ...
    inline constexpr auto get_type_names(DBType type)
    {
        ...
        iguana::for_each(T{}, [&](auto &item, auto i)
            ...
            std::string s;
            switch (type){
				case DBType::sqlite : s = sqlite_type_remmap::type_to_name(identity<U>{});
                    break;
            }
    ...
    template <class T>
    constexpr bool is_uint8_vector_v = std::is_same_v<std::vector<uint8_t>, std::remove_pointer_t<std::decay_t<T>>>;

    template <size_t N>
    inline constexpr size_t uint8_vector_size(std::vector<uint8_t> (&)[N]) { return N; }
...
#define FID(field) std::pair<std::string_view, decltype(&field)>(sqlite_wapper::get_field_name<decltype(&field)>(std::string_view(#field)), &field)
#define SID(field) get_field_name<decltype(&field)>(std::string_view(#field)).data()
}

#endif // _SQLITE_WAPPER_UTILITY_HPP_

#ifndef _SQLITE_WAPPER_SQLITE_HPP_
#define _SQLITE_WAPPER_SQLITE_HPP_
...
namespace sqlite_wapper
{
    ...
    public:
        ...
        {
            std::string sql = auto_key_map_.empty() ? generate_insert_sql<T>(false)
        ...
        std::enable_if_t<!iguana::is_reflection_v<T>, std::vector<T>> query(const Arg &s, Args &&...args)
        {
            ...
            while (true)
            {
                ...
                int index = 0;
                iguana::for_each(
                    tp, [this, &index](auto &item, auto I)
                    {
    ...
    private:
        ...
        std::string generate_createtb_sql(Args &&...args)
        {
            ...
            if constexpr (SIZE > 0)
            {
                ...
                static_assert(!(iguana::has_type<sqlite_key_t, U>::value &&
                                iguana::has_type<sqlite_auto_key_t, U>::value),
                              "should only one key");
            }
            ...
            for (size_t i = 0; i < arr_size; ++i)
            {
                ...
                for_each0(
                    tp, [&sql, &i, &has_add_field, field_name, type_name_arr, name, this](auto item)
                    {
                    if constexpr (std::is_same_v<decltype(item), sqlite_not_null_t>){
                    ...
                    else if constexpr (std::is_same_v<decltype(item), sqlite_key_t>){
                    if(!has_add_field){
                        append(sql, field_name.data(), " ", type_name_arr[i]);
                    }
                    ...
                    else if constexpr (std::is_same_v<decltype(item), sqlite_auto_key_t>){
                    if(!has_add_field){
                        append(sql, field_name.data(), " ", type_name_arr[i]);
                    }
					...
					else if constexpr (std::is_same_v<decltype(item), sqlite_unique_t>) {
						if (!has_add_field) {
							append(sql, field_name.data(), " ", type_name_arr[i]);
						}
        ...
        bool set_param_bind(T &&value, int i)
        {
            ...
            else if constexpr (is_uint8_vector_v<U>)
            {
                return SQLITE_OK == sqlite3_bind_blob(stmt_, i, value.data(), (int)value.size(), nullptr);
            }
            else
            {
                std::cout << __FUNCTION__ << ": this type has not supported yet" << std::endl;
                return false;
            }
        }
        ...
        void assign(T &&value, int i)
        {
            ...
            {
                const size_t len = (size_t)sqlite3_column_bytes(stmt_, i);
                const uint8_t *begin = (const uint8_t *)sqlite3_column_blob(stmt_, i);
                const uint8_t *end = (const uint8_t *)sqlite3_column_blob(stmt_, i) + len;
...
#endif // _SQLITE_WAPPER_SQLITE_HPP_
